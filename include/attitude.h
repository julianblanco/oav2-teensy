...
public:
  ATTITUDE();  // contstuctor
  ~ATTITUDE(); // decontstuctor
  float yaw;
  float roll;
  float pitch;
  ...
  Eigen::Matrix<double, 1, 3> derive_ang_velocity(const Eigen::Ref<const Eigen::Matrix<double, 1, 3>> &e_, const Eigen::Ref<const Eigen::Matrix<double, 1, 3>> &prev_e_);
  Eigen::Matrix<double, 3, 3> quat2rot(const Eigen::Ref<const Eigen::Matrix<double, 1, 4>> &q_);
